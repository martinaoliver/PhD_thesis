...
The models were defined by taking the original six equation model and the following species: RpaI and TetR for Node A deletion, cinI for node B deletion and cI* for node C deletion (See Fig.~\ref{fig:deletion_circuits}).
\begin{figure}[H] % h! is a placement specifier; it tries to place the image here.
    \centering
    \begin{adjustbox}{center}
...
A purely experimental control testing the circuit with node deletions was carried out by Jure Tica and Dario Bazzoli.
To ensure the pattern
%experimental node deletions

\subsection{Model deletions}
