...
Work in this chapter and in the following Chapter~\ref{chapter3} has been published in the preprint~\cite{Tica2023}.
...
\end{figure}
%TODO update legend in introduction.

A partial differential equation (PDE) system is used to describe the reaction and diffusion terms of this synthetic gene circuit (Fig.~\ref{fig:synthetic circuit_chapter2}).
...
The difference is that each have their own Ribosome Binding Site (RBS),
...
lacI and cI* concentrations will be used instead when studying how the fluorescent distributions look like spatially
...
%TODO check that turing I hopf inclusion is throughout the text.
...
to measure concentration at these points (Fig.~\ref{fig:dose_response_experimental}).  %TODO ref figure fits(Fig. S20).
...
Positive, negative or no correlations were observed in the multivariate Gaussian distributions shown in Fig.~\ref{fig:multivariate_from_fit}.
...
Finally, the model was parametrised using liquid culture data to constraint the model parameters to experimentally realistic values.
...
To simplify the model, many assumptions were taken.
...
Therefore, it unclear whether these changes would remove the capabilities of patterning.
...
%- assumption fluorescence linear not true. loqw gfp fluorescence not detected, remove thersholds. curves not matching. fit constrained incorrectly.
%TODO comment on -negative params removed.

%TODO discuss calbiration of machines. method for synbio parametrisation which does not involve calibration of machines and is universal.





%TODO comment robustness is lower than scholes but multidimensional spaces are hard to compare

%fluorescent proteins are linearly correlated with transcription factors in the same operon, and concentration of fluorescence
