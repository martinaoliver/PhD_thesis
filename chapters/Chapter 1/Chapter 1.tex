...
\begin{figure}[H] % h! is a placement specifier; it tries to place the image here.
    ...
    \caption{\textbf{Confusion matrix linking LSA output (rows) and numerical pattern outcome (columns).}. Numbers show the percentage of solutions across the rows (single linear stability output). }
...
\begin{itemize}
    ...
    \item Patterns with two peaks are also considered not to be patterned states as the two peaks might arise at the boundary for one of the diffusers due to the depletion of the other (Fig.~\ref{fig:peaks_classification} no pattern, boundary effect).
    \item Patterns with three peaks start displaying a pattern more similar to Turing's repeats, although we cannot prove the number of peaks would scale with tissue length as Turing patterns do (Fig.~\ref{fig:peaks_classification} weak pattern).
...
Confusion matrix shows the correlation between non-growing and growing systems for 9,087 parameter sets.
...
\begin{figure}[H] % h! is a placement specifier; it tries to place the image here.
    ...
    \end{adjustbox}
    \caption{\textbf{Examples of pattern transitions from non-growing domains to linearly growing domains}. Three cases are shown. For each case we show both growing and non growing solutions, as well as final snapshot (i) and time-series (ii).}
