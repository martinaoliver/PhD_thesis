...
The creation of this PostgreSQL database was necesary to explore large parameter spaces and compare between analytical and numerical results in an efficient way. %TODO add details of schema in methods
%
...
For example, patterning instabilities that are not predicted by linear stability analysis might arise due to a subcritical bifurcation~\parencite{villar, champneys}.%TODO read on this
...
Then, we present a case where linear stability analysis incorrectly predicts stationary pattern formation.
...
\begin{figure}[H] % h! is a placement specifier; it tries to place the image here.
    \centering
    \begin{adjustbox}{center}
    \includegraphics[width=1.2\textwidth]{chapters/Chapter 1/multistability_leftover} % The name of your image file; assumes it's in the same directory as your .tex file
    \end{adjustbox}
...
We then classify the different types of patterns outcomes into four.
...
\end{figure}
\section{Introducing realistic biological phenomena: Absorbing boundaries and growth}
...
More details on the encoding of boundaries can be found in Section~\ref{numerical_methods}. %TODO add boundaries to methods
...
\begin{figure}[H] % h! is a placement specifier; it tries to place the image here.
    \centering
    \begin{adjustbox}{center}
        \includegraphics[width=0.5\textwidth]{chapters/Chapter 1/growing_pattern} % The name of your image file; assumes it's in the same directory as your .tex file
    \end{adjustbox}
...
Finally, in Fig.~\ref{fig:3layer_sankey}, we can see a Sankey diagram which represents the flows from one type of pattern to another as absorbing boundaries and growth are added. Pattern classes are ranked top-to-bottom from less patterned to more.
